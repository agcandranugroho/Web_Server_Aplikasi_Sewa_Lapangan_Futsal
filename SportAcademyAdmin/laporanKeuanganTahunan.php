        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
              <div class="container-fluid">
                <div class="row mb-2">
                  <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Laporan Keuangan Per Tahun</h1>
                  </div><!-- /.col -->
                  <form method="POST">
                      <table border="0">
                      <tr>
                    ...
                    <div class="card-body">
                        <?php
                            ...
                            if(isset($_POST['cari'])){
                                ...
                                $queryHitungPenKot = mysqli_query($conn,"SELECT SUM(IF(YEAR (tanggal_pesan) = '$tahun', total_bayar, 0 )) AS TotalPenKot FROM pemesanan JOIN pembayaran ON pemesanan.id_pemesanan = pembayaran.id_pemesanan WHERE pembayaran.status = 'sudah bayar'");
                                $cekPenKot=mysqli_fetch_array($queryHitungPenKot);
                                
                                //hitung penggajian
                                $queryHitungGaji = mysqli_query($conn,"SELECT SUM(IF(YEAR(tanggal_gajian) = $tahun, gajian, 0 )) AS TotalGaji FROM penggajian ");
                                $cekGaji=mysqli_fetch_array($queryHitungGaji);
                                
                                //hitung pembelian
                                $queryHitungPem = mysqli_query($conn,"SELECT SUM(IF(YEAR(tanggal_beli) = $tahun, total_harga, 0 )) AS TotalBeli FROM pembelian ");
                                $cekPem = mysqli_fetch_array($queryHitungPem);
                        ?>
                                <h2> Pendapatan Tahun <?php echo $tahun; ?> </h2>
                                <h3>Pendapatan</h3>
